...
class LoginDialog(simpledialog.Dialog):
    ...
    def apply(self): self.result = (self.username_val.get(), self.password_val.get())
...
class JournalistApp:
    def __init__(self, root_window):
        ...
        self.root.geometry("950x800")  # Increased window height
        ...
        # (Most style configurations unchanged)
        ...
        self.style.configure("LoggedIn.TLabel", background=self.frame_bg_color, foreground="#2E7D32",
                             font=self.bold_font)
        self.style.configure("LoggedOut.TLabel", background=self.frame_bg_color, foreground="#C62828",
                             font=self.bold_font)
        ...
        self.notebook.pack(expand=True, fill='both', padx=10, pady=(10, 0), side=tk.TOP)

        self.log_text_frame = ttk.LabelFrame(root_window, text="Application Log", padding=(10, 5), style="TLabelframe")
        self.log_text_frame.pack(pady=(5, 10), padx=10, fill=tk.X, side=tk.BOTTOM, expand=False)

        self.log_text = scrolledtext.ScrolledText(self.log_text_frame, height=10, state=tk.DISABLED, wrap=tk.WORD,
                                                  **self.log_text_area_config)  # Increased height
        ...
        if not hasattr(self, 'initial_log_done'):
            self.log_message("GUI Initialized. Configure server and click 'Login'.")
            self.log_message(
                f"Using Tor SOCKS proxy: {self.tor_socks_host.get()}:{self.tor_socks_port.get()} for .onion URLs.")
            #self.log_message("SSL Certificate Verification is always DISABLED for simplicity.")
            self.initial_log_done = True

        self.update_login_status_display()

    # (Rest of the methods are identical to the previous complete version you provided,
    #  including log_message, load_config, save_config, create_tooltip, get_proxies,
    #  update_login_status_display, handle_login_logout, handle_login, perform_login_worker,
    #  handle_logout, perform_logout_worker, create_setup_tab, create_submissions_tab, create_keys_tab,
    #  upload_public_keys_to_server_threaded, upload_public_keys_to_server_worker,
    #  on_submission_select_treeview, select_private_key_file, refresh_submissions_list_threaded,
    #  refresh_submissions_list_worker, _update_submissions_tree, decrypt_selected_submission_threaded,
    #  decrypt_selected_submission_worker, generate_keys_threaded, generate_keys_worker)

    # ... PASTE ALL OTHER METHODS FROM THE PREVIOUS COMPLETE AND CORRECTED VERSION HERE ...
    # (This includes all methods from log_message down to generate_keys_worker)
    # For brevity, I will only re-paste a few key ones to show where they fit.
    # Ensure you copy ALL methods from the previous fully working version.
    ...
    def save_config(self):
        ...
        config_data = {"server_url": self.server_url.get(), "last_username": self.last_username.get(),
                       "tor_socks_host": self.tor_socks_host.get(), "tor_socks_port": port_val}
        try:
    ...
    def update_login_status_display(self):
        if hasattr(self, 'login_status_label'):
            if self.is_logged_in:
                status_text = f"Logged in as: {self.current_journalist_username.get()}"
                self.login_status_label.config(text=status_text, style="LoggedIn.TLabel")
                if hasattr(self, 'login_button'): self.login_button.config(text="Logout")
            else:
                status_text = "Status: Not logged in"
                self.login_status_label.config(text=status_text, style="LoggedOut.TLabel")
                if hasattr(self, 'login_button'): self.login_button.config(text="Login")
    ...
    def handle_login(self):
        ...
        if dialog.result:
            ...
            if not username or not password: messagebox.showerror("Login Failed",
                                                                  "Username and password cannot be empty.",
                                                                  parent=self.root); return
            self.last_username.set(username);
            self.save_config()
    ...
    def perform_login_worker(self, username, password):
        ...
        try:
            ...
            self.log_message(f"Login response status: {response.status_code}", "DEBUG");
            self.log_message(f"Login response headers: {response.headers}", "DEBUG");
            self.log_message(f"Login response text (first 500 chars): {response.text[:500]}", "DEBUG_DATA")
            if response.url == login_url or "/login" in response.url:
                if "Invalid username or password" in response.text or "error" in response.text.lower() or response.status_code != 200:
                    self.log_message(f"Login failed for {username} (server indicated failure or unexpected redirect).",
                                     "ERROR")
                    self.is_logged_in = False;
            ...
            self.root.after(0, self.update_login_status_display);
            self.root.after(0, self.refresh_submissions_list_threaded)
        except requests.exceptions.HTTPError as http_e:
            self.is_logged_in = False;
            self.current_journalist_username.set("Not logged in");
            err_msg = f"Login HTTP error for {username}: {http_e.response.status_code}.";
            self.log_message(f"Login HTTP error response text: {http_e.response.text[:500]}", "DEBUG_DATA")
            try:
                server_err_json = http_e.response.json(); flash_msg = server_err_json.get("flash_message");
            except json.JSONDecodeError:
                flash_msg = None
            if flash_msg:
                err_msg += f" Server message: {flash_msg}"
            elif "Invalid username or password" in http_e.response.text:
    ...
    def perform_logout_worker(self):
        if not self.is_logged_in: self.log_message("Not logged in, logout unnecessary.", "INFO"); return
        ...
        request_timeout = 90
        try:
            response = self.session.get(logout_url, proxies=current_proxies, verify=ssl_verify, timeout=request_timeout)
    ...
    def create_setup_tab(self):
        ...
        self.login_status_label = ttk.Label(login_status_frame, text="Status: Not logged in", style="LoggedOut.TLabel")
        self.login_status_label.pack(side=tk.LEFT, padx=5, pady=5, fill=tk.X, expand=True)
    ...
    def upload_public_keys_to_server_threaded(self):
        if not self.is_logged_in: self.log_message("Login required to upload public keys.",
                                                   "ERROR"); messagebox.showerror("Login Required",
                                                                                  "Please login first.",
                                                                                  parent=self.root); return
        selected_key_files = filedialog.askopenfilenames(parent=self.root, title="Select Public Key Files",
                                                         initialdir=DEFAULT_PUBLIC_KEYS_FOR_SERVER_DIR,
                                                         filetypes=(("Public Key files", "*.pem *.pub"),
                                                                    ("All files", "*.*")))
        if not selected_key_files: self.log_message("No public key files selected."); return
    ...
    def upload_public_keys_to_server_worker(self, key_files_to_upload):
        ...
        try:
            ...
            response = self.session.post(target_url, json=payload_to_send, headers=headers_req, proxies=current_proxies,
                                         verify=ssl_verify, timeout=request_timeout)
            response.raise_for_status();
            ...
            if result_data.get('details'):
                for detail in result_data['details']: self.log_message(
            ...
            err_msg = f"HTTP error uploading keys: {http_e.response.status_code}."; self.log_message(err_msg,
                                                                                                     "ERROR"); self.root.after(
                0, lambda em=err_msg: messagebox.showerror("Upload Error", em, parent=self.root))
        except requests.exceptions.RequestException as req_e:
            self.log_message(f"Network error uploading keys: {req_e}", "ERROR"); self.root.after(0, lambda
                er=req_e: messagebox.showerror("Network Error", f"Could not upload keys: {er}", parent=self.root))
        except Exception as e_gen:
            self.log_message(f"Unexpected error during key upload: {e_gen}",
                             "ERROR"); traceback.print_exc(); self.root.after(0, lambda eg=e_gen: messagebox.showerror(
                "Error", f"Unexpected error: {eg}", parent=self.root))

    def on_submission_select_treeview(self, event):
    ...
    def refresh_submissions_list_threaded(self):
        if not self.is_logged_in: self.log_message("Login required to refresh submissions.", "INFO"); return
    ...
    def refresh_submissions_list_worker(self):
        ...
        try:
            ...
            response = self.session.get(target_url, proxies=current_proxies, verify=ssl_verify, timeout=request_timeout)
            response.raise_for_status()
            content_type = response.headers.get('content-type', '').lower()
            if 'application/json' in content_type:
                data = response.json();
                self.log_message(f"Submissions raw data: {str(data)[:200]}", "DEBUG_DATA")
                self.root.after(0, lambda d=data: self._update_submissions_tree(d))
            else:
                self.log_message(f"Unexpected content type for submissions: {content_type}", "ERROR");
                self.log_message(f"Response text (first 500 chars): {response.text[:500]}", "DEBUG_DATA")
                self.root.after(0, lambda: messagebox.showerror("Refresh Error",
                                                                "Received non-JSON response. You might not be properly logged in.",
                                                                parent=self.root))
                self.is_logged_in = False;
                self.root.after(0, self.update_login_status_display)
        except requests.exceptions.HTTPError as http_e:
            err_msg = f"HTTP error refreshing: {http_e.response.status_code}.";
            self.log_message(err_msg, "ERROR");
            self.root.after(0, lambda em=err_msg: messagebox.showerror("Network Error", em, parent=self.root))
            if http_e.response.status_code in [401, 403]: self.is_logged_in = False; self.root.after(0,
                                                                                                     self.update_login_status_display)
        except requests.exceptions.RequestException as req_e:
            self.log_message(f"Network error refreshing: {req_e}", "ERROR"); self.root.after(0, lambda
                er=req_e: messagebox.showerror("Network Error", f"Could not refresh: {er}", parent=self.root))
        except json.JSONDecodeError as json_e:
            self.log_message(f"Invalid JSON response (refreshing submissions): {json_e}", "ERROR");
            self.log_message(f"Response text that caused JSON error (first 500 chars): {response.text[:500]}",
                             "DEBUG_DATA")
            self.root.after(0, lambda: messagebox.showerror("Server Error", "Invalid JSON response from server.",
                                                            parent=self.root))
        except Exception as ex:
            self.log_message(f"Unexpected error refreshing: {ex}", "ERROR"); traceback.print_exc(); self.root.after(0,
                                                                                                                    lambda
                                                                                                                        e=ex: messagebox.showerror(
                                                                                                                        "Error",
    ...
    def _update_submissions_tree(self, data):
        ...
        if data and "submissions" in data and isinstance(data["submissions"], list):
            ...
            if s_list:
                for item_info in s_list: s_id = item_info.get("id", "UnknownID"); k_hint = item_info.get("key_hint",
                                                                                                         "N/A"); self.submissions_tree.insert(
                    "", tk.END, values=(s_id, k_hint))
    ...
    def decrypt_selected_submission_threaded(self):
        if not self.is_logged_in: messagebox.showwarning("Login Required", "Please login to decrypt submissions.",
                                                         parent=self.root); return
        if not self.selected_submission_id.get(): messagebox.showwarning("No Selection", "Please select a submission.",
                                                                         parent=self.root); return
        if not self.private_key_path.get(): messagebox.showwarning("No Key", "Please select an RSA private key.",
                                                                   parent=self.root); return
        ...
        try:
            ...
            self.log_message(f"Error reading private key file: {e}", "ERROR"); messagebox.showerror("Key Error",
                                                                                                    f"Could not read private key: {e}",
                                                                                                    parent=self.root); return
    ...
    def decrypt_selected_submission_worker(self, sub_id, private_key_pem_content, priv_key_password):
        ...
        try:
            ...
            resp_pkg = self.session.get(package_info_url, proxies=current_proxies, verify=ssl_verify,
                                        timeout=pkg_timeout)
            ...
            for name, url_val in urls_to_fetch.items():
                current_comp_timeout = file_timeout if name == "file" else comp_timeout
                resp_comp = self.session.get(url_val, proxies=current_proxies, verify=ssl_verify,
                                             timeout=current_comp_timeout)
            ...
            decrypted_aes_key = crypto_utils.decrypt_rsa(encrypted_aes_key_data, private_key_pem_content,
                                                         priv_key_password)
            if not decrypted_aes_key: self.log_message("Failed to decrypt AES key.", "ERROR"); self.root.after(0,
            ...
            if dec_orig_fname_bytes: original_filename = dec_orig_fname_bytes.decode('utf-8', errors='replace')
            decrypted_file_data = crypto_utils.decrypt_aes_gcm(encrypted_file_data, decrypted_aes_key)
            if not decrypted_file_data: self.log_message("Failed to decrypt file data.", "ERROR"); self.root.after(0,
                                                                                                                   lambda: messagebox.showerror(
                                                                                                                       "Decryption Error",
                                                                                                                       "File data decryption failed.",
                                                                                                                       parent=self.root)); return
            ...
            err_msg = f"HTTP error decrypting: {http_e.response.status_code}.";
            self.log_message(err_msg, "ERROR");
            self.root.after(0, lambda em=err_msg: messagebox.showerror("Network Error", em, parent=self.root))
            if http_e.response.status_code in [401, 403]: self.is_logged_in = False; self.root.after(0,
                                                                                                     self.update_login_status_display)
        except requests.exceptions.RequestException as req_e:
            self.log_message(f"Network error decrypting: {req_e}", "ERROR"); self.root.after(0, lambda
                er=req_e: messagebox.showerror("Network Error", f"Could not download: {er}", parent=self.root))
        except Exception as e_gen:
            self.log_message(f"Unexpected error in decryption: {e_gen}",
                             "ERROR"); traceback.print_exc(); self.root.after(0, lambda be=e_gen: messagebox.showerror(
                "Error", f"Unexpected error: {be}", parent=self.root))
    ...
    def generate_keys_worker(self, num_keys, key_prefix, password):
        ...
        for i in range(1, num_keys + 1):
            try:
                ...
                self.log_message(f"Error generating key pair {i}: {e}", "ERROR"); self.root.after(0, lambda err_val=e,
                                                                                                            idx_val=i: messagebox.showerror(
                    "Key Generation Error", f"Failed for pair {idx_val}: {err_val}", parent=self.root)); return
        ...
        def update_gui_after_gen():
            ...
            self.log_message(f"{num_keys} pairs generated.", "SUCCESS");
        ...
        if os.path.exists(icon_path):
            try:
                if icon_path.endswith('.png'):
                    ...
                    root.iconbitmap(default=icon_path); icon_found = True; break
