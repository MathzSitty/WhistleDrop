...
if __name__ == "__main__":
    ...
    # This ensures the DB and tables (including journalists) are ready.
    try:
        # A simple check to confirm DB connection is possible, though import should handle it.
        conn = key_manager.get_db_connection()
        conn.close()
    except Exception as db_init_e:
        print(f"CRITICAL: Could not connect to or initialize key database: {db_init_e}")
        print("Please ensure the server's data directory is writable and the database can be accessed.")
        sys.exit(1)
    main()